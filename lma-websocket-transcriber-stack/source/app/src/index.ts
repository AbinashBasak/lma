...
import { createWavHeader, posixifyFilename, normalizeErrorForLogging, getClientIP } from './utils';
...
const registerHandlers = (clientIP: string, ws: WebSocket, request: FastifyRequest): void => {
	...
	ws.on('close', (code: number) => {
		server.log.debug(`[ON WSCLOSE]: [${clientIP}] Received Websocket close message from the client. Closing the connection.`);
	...
	try {
		...
		server.log.error(
			`[${callMetaData.callEvent}]: [${
				callMetaData.callId
			}] - Error uploading ${sourceFile} to S3: ${normalizeErrorForLogging(err)}`
		);
	} finally {
		fileStream.destroy();
	}
...
const deleteTempFile = async (callMetaData: CallMetaData, sourceFile: string) => {
	try {
		...
		server.log.error(
			`[${callMetaData.callEvent}]: [${
				callMetaData.callId
			}] - Error deleting tmp file ${sourceFile} : ${normalizeErrorForLogging(err)}`
		);
	}
};
