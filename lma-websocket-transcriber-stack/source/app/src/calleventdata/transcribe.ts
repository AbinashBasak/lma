	...
	try {
		...
		server.log.debug(
			`[${callEvent.EventType}]: ${callEvent.CallId} - Error writing ${
				callEvent.EventType
			} Call Event to KDS : ${normalizeErrorForLogging(error)} Event: ${JSON.stringify(callEvent)}`
		);
	}
};
...
export const writeCallStartEvent = async (callMetaData: CallMetaData, server: FastifyInstance): Promise<void> => {
	const callStartEvent: CallStartEvent = {
		...
		meetingTopic: callMetaData.meetingTopic,
	...
	const startTranscribeSession = async (retryCount = 0): Promise<void> => {
		...
		try {
			...
			if (isTCAEnabled) {
				...
				const response = await transcribeClient.send(new StartStreamTranscriptionCommand(tsParams));
				sessionId = response.SessionId;
...
export const writeTranscriptionSegment = async function (
	...
	if (transcribeMessageJson.Transcript?.Results && transcribeMessageJson.Transcript?.Results.length > 0) {
		...
		if (result.Alternatives && result.Alternatives.length > 0) {
			const speakerName = result.ChannelId === 'ch_0' ? callMetadata.activeSpeaker : callMetadata?.agentId ?? 'n/a';
			const segments = processTranscriptionResults(speakerName, result, callMetadata, server);
			...
			for (const segment of Object.values(segments)) {
				...
				try {
					...
					server.log.error(
						`[${kdsObject.EventType}]: [${callMetadata.callId}] - Error writing ${
							kdsObject.EventType
						} to KDS : ${normalizeErrorForLogging(error)} KDS object: ${JSON.stringify(kdsObject)}`
					);
				}
			}
		}
