...
def invoke_transcript_lambda_hook(message: Dict[str, Any]):
    if (
        message.get("IsPartial") == False
        or TRANSCRIPT_LAMBDA_HOOK_FUNCTION_NONPARTIAL_ONLY == False
    ):
...
async def execute_process_event_api_mutation(
    ...
    message["MeetingTopic"] = message.get("MeetingTopic", "")
    message["UserName"] = message.get("UserName", "")
    ...
    if event_type == "START":
        ...
        for normalized_message in normalized_messages:
            ...
            if (
                (IS_LEX_AGENT_ASSIST_ENABLED or IS_LAMBDA_AGENT_ASSIST_ENABLED)
                and (
